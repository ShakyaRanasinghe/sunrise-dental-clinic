...
| **GAP-ADM-14** | Staff login pages wear the public navigation and sign-out lands everywhere on `/` | **Fixed** — bare branded bar on staff logins + `/staff` (theme + role label); role-aware sign-out (`LogoutServlet`, `header.jspf`, `AbstractLoginServlet`, `StaffPortalServlet`) | **Fixed** |
| **GAP-ADM-13** | The Accounts table has no role filter, and appointments tables sort one fixed way | **Fixed** — `roleFilter` on Accounts; `sort` on day/billing (time, dentist) and complaints (newest/oldest); sort kept across day-view cancels (`AccountsServlet`, `ReceptionDayServlet`, `BillingPageServlet`, `AdminComplaintsServlet`, JSPs) | **Fixed** |
