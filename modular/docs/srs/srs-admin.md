...
| **GAP-FTB-12** | There is a single shared help page (`/help`) written for the public; the administrator uses the system differently (reports, accounts, treatments, clinic settings, complaints, audit) and has no help page for that scenario, rooted in the project's SRS/docs | Not yet addressed — an **administrator-specific help page** referencing the SRS docs, reached from the admin sign-in page; the shared `/help` stays for the public. (Parent gap GAP-FTB-12 in `gap-checklist.md`) | **Open** |
| **GAP-FTB-14** | Profile/settings pages render every editable field as an in-page form at once; the user wants a **read-only view by default** with an **Edit** control revealing only the changeable fields, and every successful update in the system (admin editing clinic identity, treatments, accounts, resolving complaints) to confirm with a **sub-window ("successfully updated")** rather than only a top-of-page notice | Not yet addressed — read-only-by-default edit pattern plus a success sub-window after any update, applied to the admin's update points. (Parent gap GAP-FTB-14 in `gap-checklist.md`) | **Open** |
| **GAP-PAT-29** | The booking route lets a patient inspect a treatment's full details only if the patient role surfaces them; the administrator authors each treatment's description (`/admin/treatments`) | Not yet addressed as a patient-facing feature — persists here only as the authoring side; the surfacing work is tracked as GAP-PAT-29 in `srs-patient.md`. No admin change required | **Open** |
