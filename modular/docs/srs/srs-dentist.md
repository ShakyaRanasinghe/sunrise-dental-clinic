...
|---|---|---|
| **FR-DEN-01** | `/login/dentist` must present username and password only (GAP-ADM-12; email still works). No registration link — a dentist's account is issued by an administrator (ASM-05) | Built |
...
| **GAP-DEN-15** | A dentist never sees the revenue share the administrator configured: the dashboard shows the consultation fee but nothing says what fraction of each treatment price comes to them | **Fixed** — the profile states the live configured share as a rate ("You receive N%…"), never an earnings figure per §9 (`DentistProfileServlet`, `ReferenceService.dentistSharePercent`, `dentist-profile.jsp`) | **Fixed** |
| **GAP-ADM-12** | Staff sign in with usernames (see GAP-ADM-12 in `srs-admin.md`) | **Fixed** — dentist portal asks for a username; FR-DEN-01 updated | **Fixed** |
| **GAP-ADM-14** | Staff doors show no public navigation; sign-out returns here to `/staff` (see GAP-ADM-14 in `srs-admin.md`) | **Fixed** | **Fixed** |
| **GAP-DEN-16** | The availability screen leads with "Treatments I offer" while the time windows sit below, and the windows table is bare counts | **Fixed** — windows lead with day totals ("N booked · M still open"), green open pills and Full for exhausted windows; treatments list below (`dentist-availability.jsp`) | **Fixed** |
| **GAP-DEN-14** | Only the patient has a **"My details"** profile section; a dentist has nowhere to see or edit their own profile (name, specialisation, phone) as the public cards show it | **Fixed** — "My details" in the account menu (`/dentist/profile`): read-only table with an Edit control for name/specialisation/phone (phone validated), consultation fee shown disabled as administrator-owned, saves confirm in a sub-window (`DentistProfileServlet`, `ReferenceService.updateOwnDetails`, `scheduling/dentist-profile.jsp`, `header.jspf`, `web.xml`). The older phone card on the availability screen (and its POST path) is removed — the profile is now the single place for it | **Fixed** |
