...
| **GAP-FTB-12** | There is a single shared help page (`/help`) written for the public; a dentist uses the system differently (schedule, publishing availability, toggling offered treatments, recording diagnoses) and has no help page for that scenario, rooted in the project's SRS/docs | Not yet addressed — a **dentist-specific help page** referencing the SRS docs, reached from the dentist sign-in page; the shared `/help` stays for the public. (Parent gap GAP-FTB-12 in `gap-checklist.md`) | **Open** |
| **GAP-FTB-14** | Profile/settings pages render every editable field as an in-page form at once; the user wants a **read-only view by default** with an **Edit** control revealing only the changeable fields, and every successful update in the system (a dentist updating their phone, toggling treatments, publishing availability) to confirm with a **sub-window ("successfully updated")** rather than only a top-of-page notice | Not yet addressed — read-only-by-default edit pattern plus a success sub-window after any update, applied to the dentist's update points. (Parent gap GAP-FTB-14 in `gap-checklist.md`) | **Open** |
