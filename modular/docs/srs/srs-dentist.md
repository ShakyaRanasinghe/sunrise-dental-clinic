...
| **GAP-FTB-14** | Profile/settings pages render every editable field as an in-page form at once; the user wants a **read-only view by default** with an **Edit** control revealing only the changeable fields, and every successful update in the system (a dentist updating their phone, toggling treatments, publishing availability) to confirm with a **sub-window ("successfully updated")** rather than only a top-of-page notice | **Fixed** — the phone card on the availability dashboard is now read-only with an Edit disclosure, and saving confirms in the shared `.sub-window.open` success window (`dentist-availability.jsp`, `app.css`). (Parent gap GAP-FTB-14 in `gap-checklist.md`) | **Fixed** |
