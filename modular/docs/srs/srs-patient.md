...
| **GAP-PAT-11** | The create-account form gave no way to see what was being typed: both password fields were `type=password` with no show/hide control, so on a phone a patient had no way to check a passphrase before submitting — the most error-prone field was the one they could not read back | Added a **Show/Hide password** toggle beside the password and confirmation fields (`register.jsp`). It is a plain button that only flips `input[type]` between `password` and `text`; it never touches the value, and a browser with no JavaScript simply has no button, so the fields keep the secure `password` default. Toggle state tracked with `aria-pressed` | **Fixed** |
