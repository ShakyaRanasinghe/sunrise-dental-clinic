...
| **GAP-PAT-22** | Public home page: every service (`.service-item`) carries a **"Book this service &rarr;"** link in its body, inviting a sign-up from the middle of the page | Not yet addressed — the link is to be removed so a service description is shown without a mid-page booking call-to-action (`access/home.jsp`) | **Open** |
| **GAP-PAT-23** | Public home page: the **"Our dentists"** cards are `<details>` rows, so a visitor has to expand each one (they open with the first expanded) to see the fee, phone and rating; the user wants the whole card visible without expanding | Not yet addressed — each dentist card is to render fully without the `<details>/<summary>` toggle (`access/home.jsp`) | **Open** |
| **GAP-PAT-24** | The signed-in **patient dashboard navigation has no Help link** — every role's `navigation()` currently omits it — so a logged-in patient has no in-app help | Not yet addressed — add a **Help** entry to the patient dashboard nav, linking to the role's own help page (GAP-FTB-12) (`PatientPolicy`, `header.jspf`) | **Open** |
| **GAP-PAT-25** | The **"Raise a concern"** page opens with a tall explanatory block — the `<h2>What happened</h2>` heading, the **"Who reads this: the clinic's administrator, and nobody else…"** notice, and the **"Choose the dentist your concern is about. The form opens when you do."** line — more text than a patient needs to see to proceed | Not yet addressed — the intro block is to be removed so the page leads straight into the per-dentist concern list and form (`patient-complaints.jsp`). FR-CMP-12 wording is not removed, only this presentation block | **Open** |
| **GAP-PAT-26** | The self-registration form is **narrow (`.narrow` card) and tall**, forcing a new patient to scroll; it also explains the obvious (phone helper text **"So the clinic can reach you about an appointment (e.g. 077 123 4567)."**) and reassures about a non-present field (**"Nothing medical is asked here. Allergies and medications are added from your profile once you have signed in."**) | Not yet addressed — widen the card, lay fields **two per line** to reduce height below the scroll, and delete the two sentences (`access/register.jsp`, `app.css`) | **Open** |
| **GAP-PAT-27** | The patient role is designed to record **diagnosis / dental-history details**, but the profile screen has **no such field** — there is nowhere to add them even after entering the profile | Not yet addressed — add a diagnosis/history details field to the patient profile that can be edited and saved (`profile.jsp`, `PatientProfileServlet`/service, schema) | **Open** |
| **GAP-PAT-28** | Booking page: once a dentist and time are chosen the submit button reads **"Confirm booking"** with no sense of which doctor or what is being confirmed; the page still shows a **"2. Dentist and date"** picker card below the times (redundant on a page the patient is already on); and the **treatment picker is shown above the time slots** | Not yet addressed — button to read **"Proceed appointment &lt;dentist name&gt;"**, the **"2. Dentist and date"** card removed, and the **time slots shown first** under a heading naming the doctor's available times, with the treatment list below (`book.jsp`) | **Open** |
| **GAP-PAT-29** | Each treatment type in the booking list shows its name, short description and price but has **no way to read its full details**, even though the administrator authors a description | Not yet addressed — give each treatment an **info icon/link** that opens the administrator-written description in a sub-window (`book.jsp`, `app.css`) | **Open** |
| **GAP-FTB-11** | The site footer labels the running build **"Release v1.0.1"** | Not yet addressed — change the label from **"Release"** to **"Version"** (`footer.jspf`) | **Open** |
| **GAP-FTB-12** | There is a **single shared help page (`/help`)**; each role uses the system differently and should have a **help page specific to its own scenario**, rooted in the project's SRS/docs. Each role's sign-in page (and the sign-up page) should link to its own help; the **help link on the `/staff` portal should be removed** | Not yet addressed — one role-specific help page per role referencing the SRS/docs; patient login (`/login/patient`) and register (`/register`) link to the patient help; remove the shared help link from `staff-portal.jsp`. Affects this role plus `srs-reception.md`, `srs-dentist.md`, `srs-admin.md` | **Open** |
| **GAP-FTB-13** | The public help page's contents card is headed **"Jump to a topic"**, a label that reads oddly for a page of steps | Not yet addressed — replace the label with something simpler/appropriate (`shared/help.jsp`) | **Open** |
| **GAP-FTB-14** | Profile pages render **every editable field as an in-page form at once**; the user wants a **read-only view by default** with an **Edit** control that reveals **only the fields that role may change**, and every successful update anywhere in the system (profile, availability, settings) should confirm with a **sub-window ("successfully updated")** rather than only a top-of-page notice | Not yet addressed — read-only-by-default profile with an Edit reveal across all roles, plus a success sub-window after any update; the patient's is `profile.jsp`, but the pattern is system-wide. Affects this role plus `srs-reception.md`, `srs-dentist.md`, `srs-admin.md` | **Open** |
