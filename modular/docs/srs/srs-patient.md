...
| **GAP-PAT-14** | The sign-in page that every visitor lands on from the public header (`/login`) listed **all** staff portals as visible cards — Reception, Dentist and (by design) a patient card. A member of the public could read the URLs `/login/reception` and `/login/dentist` straight off the homepage flow, even though those doors exist for staff only and no patient has business there. Seeing that a staff surface exists (and where) is a confidentiality leak, not just noise | The public chooser now lists **only the patient portal**. Staff doors still work — `/login/reception` and `/login/dentist` (and the already-hidden `/login/admin`, `FR-ADM-03`) answer normally, but they are no longer advertised: a staff member reaches their own door by the URL issued to them. Direct links from the hero and the register page (`/login/patient`) are untouched, so existing patient flows do not change | **Fixed** |
