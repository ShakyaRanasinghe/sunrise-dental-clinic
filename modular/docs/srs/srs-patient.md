...
| **GAP-FTB-14** | Profile pages render **every editable field as an in-page form at once**; the user wants a **read-only view by default** with an **Edit** control that reveals **only the fields that role may change**, and every successful update anywhere in the system (profile, availability, settings) should confirm with a **sub-window ("successfully updated")** rather than only a top-of-page notice | **Fixed** — new `.sub-window.open` style; patient profile, dentist phone (now read-only + Edit), admin clinic identity and treatment catalogue all confirm saves in it. Affects this role plus `srs-reception.md`, `srs-dentist.md`, `srs-admin.md` | **Fixed** |
