...
| **GAP-FTB-06** | The booking screen chose a treatment from a bare dropdown, so a patient whose need matched no listed procedure had no honest way to book — and reception's walk-in booking sent the same. The visit a patient actually wants was neither capturable nor recorded | The dropdown became a **single-treatment radio checklist** (name, description, price per choice) with an **"Other (describe…)"** option whose free text is stored as the booking reason on the appointment. The server requires a treatment or a reason, and the reason round-trips on the appointment record. Verified live: an "Other" booking stored `patient_reason` with `treatment_id` NULL. `appointment.patient_reason`, `AppointmentService.book(…, patientReason)`, `BookAppointmentServlet`, `book.jsp`, `app.css` | **Fixed** |
| **GAP-PAT-22** | Public home page: every service (`.service-item`) carries a **"Book this service &rarr;"** link in its body, inviting a sign-up from the middle of the page | **Fixed** — the link is removed; a service shows name + description only (`access/home.jsp`) | **Fixed** |
| **GAP-PAT-23** | Public home page: the **"Our dentists"** cards are `<details>` rows, so a visitor has to expand each one (they open with the first expanded) to see the fee, phone and rating; the user wants the whole card visible without expanding | **Fixed** — each dentist renders as a full static card; the expand chevron CSS is replaced with static summary rules (`access/home.jsp`, `app.css`) | **Fixed** |
| **GAP-PAT-24** | The signed-in **patient dashboard navigation has no Help link** — every role's `navigation()` currently omits it — so a logged-in patient has no in-app help | **Fixed** — the patient nav ends in **Help → `/help/patient`** (`PatientPolicy`); the other roles gain their own Help entries under GAP-FTB-12 | **Fixed** |
| **GAP-PAT-25** | The **"Raise a concern"** page opens with a tall explanatory block — the `<h2>What happened</h2>` heading, the **"Who reads this: the clinic's administrator, and nobody else…"** notice, and the **"Choose the dentist your concern is about. The form opens when you do."** line — more text than a patient needs to see to proceed | **Fixed** — the heading, notice and preamble are removed; the card opens as "Your visits" straight into the per-dentist list (`patient-complaints.jsp`). The page subtitle still names the administrator as the reader (FR-CMP-12) | **Fixed** |
| **GAP-PAT-26** | The self-registration form is **narrow (`.narrow` card) and tall**, forcing a new patient to scroll; it also explains the obvious (phone helper text **"So the clinic can reach you about an appointment (e.g. 077 123 4567)."**) and reassures about a non-present field (**"Nothing medical is asked here. Allergies and medications are added from your profile once you have signed in."**) | **Fixed** — the card is wider (`.register-wide`, 720px), fields pair two-per-line, both sentences are deleted, and a "Need help signing up?" link points at `/help` (`access/register.jsp`, `app.css`) | **Fixed** |
| **GAP-PAT-27** | The patient role is designed to record **diagnosis / dental-history details**, but the profile screen has **no such field** — there is nowhere to add them even after entering the profile | **Fixed** — `patient.diagnosis_details` added (`schema.sql`, migrated live) and threaded through `Patient`/`PatientResponse`/`PatientDao`/`PatientService.ProfileUpdate`/`PatientProfileServlet`; shown read-only and edited via textarea on `profile.jsp` | **Fixed** |
| **GAP-PAT-28** | Booking page: once a dentist and time are chosen the submit button reads **"Confirm booking"** with no sense of which doctor or what is being confirmed; the page still shows a **"2. Dentist and date"** picker card below the times (redundant on a page the patient is already on); and the **treatment picker is shown above the time slots** | **Fixed** — one card headed "Available times with … on …" (slots first, treatment picker below), the submit reads **"Proceed appointment with …"**, and the picker card is replaced by a "Choose a different dentist or date" link (`book.jsp`) | **Fixed** |
| **GAP-PAT-29** | Each treatment type in the booking list shows its name, short description and price but has **no way to read its full details**, even though the administrator authors a description | **Fixed** — each treatment carries an info mark opening its full description + price in a `:target` sub-window (`book.jsp`, `.treatment-info` in `app.css`) | **Fixed** |
| **GAP-PAT-31** | Opening Book with no open slots anywhere renders a bare title on the patient page, and the walk-in flow names no doctors | **Fixed** — a no-doctors card on `book.jsp` and a no-doctors notice on `walkin.jsp`, both asking to check later (`book.jsp`, `walkin.jsp`) | **Fixed** |
| **GAP-PAT-32** | A concern can only name a dentist the patient has seen — a general concern about the clinic is impossible, and every concern carries the patient's identity | **Fixed** — General card (`action=general`) needing no dentist or history, with an anonymity notice; stored NULL/NULL so no lookup can tie it back; the empty-history text points at it (`patient-complaints.jsp`, `PatientComplaintsServlet`, `ComplaintService.raiseGeneral`, schema) | **Fixed** |
| **GAP-PAT-33** | Patient ids are database UUIDs, unreadable over the phone and unquotable at the desk | **Fixed** — `patient.patient_no` minted by `PersonNumberGenerator` on self-reg and walk-in (UUID PKs untouched); `patientNumber()` falls back to the id; existing rows backfilled by enrol date | **Fixed** |
| **GAP-PAT-30** | A finished visit's info shows charges but not **what the dentist did**: opening a finished appointment (dashboard receipt view, full receipt page) never shows the dentist's recorded description | **Fixed** — the dashboard receipt view prints the gated diagnosis row and the full receipt prints "Dentist's note" only for the patient viewer (`showClinical`); reception/admin copies stay clinical-free (`patient-home.jsp`, `billing/receipt.jsp`, `ReceiptServlet`, `BillResponse.diagnosis`) | **Fixed** |
| **GAP-FTB-11** | The site footer labels the running build **"Release v1.0.1"** | **Fixed** — the footer reads **"Version v…"** (`footer.jspf`) | **Fixed** |
| **GAP-FTB-12** | There is a **single shared help page (`/help`)**; each role uses the system differently and should have a **help page specific to its own scenario**, rooted in the project's SRS/docs. Each role's sign-in page (and the sign-up page) should link to its own help; the **help link on the `/staff` portal should be removed** | **Fixed** — `HelpServlet` dispatches `/help/<role>` to four new role pages (`shared/help-<role>.jsp`, each grounded in its role SRS); `web.xml` maps `/help/*` (public via the `/help` filter prefix); every policy nav ends in its own Help; staff logins link their own help, register links `/help`, and `/staff` offers none | **Fixed** |
| **GAP-FTB-13** | The public help page's contents card is headed **"Jump to a topic"**, a label that reads oddly for a page of steps | **Fixed** — the card is headed **"Help topics"** on the shared page and all four role pages (`shared/help*.jsp`) | **Fixed** |
| **GAP-FTB-14** | Profile pages render **every editable field as an in-page form at once**; the user wants a **read-only view by default** with an **Edit** control that reveals **only the fields that role may change**, and every successful update anywhere in the system (profile, availability, settings) should confirm with a **sub-window ("successfully updated")** rather than only a top-of-page notice | **Fixed** — new `.sub-window.open` style; **every** update point confirms in it: patient profile/book/cancel/rate/notes/concerns, dentist phone + toggles + recorded treatments, reception cancels + walk-in register + published availability, admin clinic/treatments/complaints/accounts. Warnings and errors stay inline. Affects this role plus `srs-reception.md`, `srs-dentist.md`, `srs-admin.md` | **Fixed** |
