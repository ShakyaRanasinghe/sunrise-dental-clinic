...
| **GAP-REC-11** | The walk-in register form has no contact-number field at all: a patient registered at the desk is stored without a phone, even though self-registration accepts one (GAP-REC-08 validates a number when present). Reception cannot record the number at the point of registration, so a walk-in patient is never reachable and — with no phone on the record — hard to match to a later self-registered account | **Open** — add a contact-number input to the walk-in register form (see `reception/patients` / walk-in registration), validated through the same shared `PhoneNumbers` rule as every other number entry, consistent with the new mandatory phone in self-registration (GAP-PAT-20) | **Open** |
| **GAP-REC-12** | Two or more patients can carry the same name, and with nothing else unique shown in the **Patients** list reception cannot tell which row is the person in front of them — the same name appears several times with no distinguishing value, so a walk-in or a repeat visitor is easily booked against the wrong record. There is no patient identifier surfaced anywhere in the list | **Open** — give each patient a unique identifier persisted in the database and surface it as the **leftmost column** of the reception Patients table, so a same-named offset record can be told apart at a glance. The identifier should be stable and friendly for the desk to read out loud (e.g. a short patient number/ID) rather than a raw primary key | **Open** |
