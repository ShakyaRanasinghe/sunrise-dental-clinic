...
|---|---|---|
| **FR-REC-01** | `/login/reception` must present username and password only (GAP-ADM-12; email still works). No registration link — staff accounts are created by an administrator (ASM-05) | Built |
...
| **GAP-ADM-12** | Staff sign in with usernames (see GAP-ADM-12 in `srs-admin.md`) | **Fixed** — reception portal asks for a username; FR-REC-01 updated | **Fixed** |
| **GAP-ADM-13** | Reception tables sort one fixed way (see GAP-ADM-13 in `srs-admin.md`) | **Fixed** — day view and billing sort by time asc/desc and dentist via header links | **Fixed** |
